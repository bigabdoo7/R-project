;	function formErrors(data, form, errorCss){
	...
	function formSuccess(data,form){
		...
		if(data != '') {
	    	...
	    	$.each(data, function(key, val) {
	    		if(key != 'success') {
	   				$(form+" #"+key+"_em_").html(val.toString());
	   				$(form+" #"+key+"_em_").show();
	   				$("#"+key).parent().addClass("row success");
	   				summary = summary + val.toString() + "<br />";
	   			}
	   		});
	    	summary+="</div>";
		...
		$('.alert-box').delay(8000).fadeOut(2000);
	...
	function replaceSuccess(data,form){
		...
		$(form+ ' fieldset').html('');	
		if(data != '') {
	   		summary="<div class=\"alert-box success\" style=\"margin:5px;position:relative\">";
	    	$.each(data, function(key, val) {
	    		if(key != 'success') {
	   				$(form+" #"+key+"_em_").html(val.toString());
	   				$(form+" #"+key+"_em_").show();
